 ...
 */
int addCommandToHistory(char command[],int current) //void to int
{
    ...
    else
    {
        ...
        if(command[0]=='r')
        {
            if(command[1]=='r')
            {
                ...
                strcpy(command,g_history[(g_command_count)% HISTORY_SIZE]);
            }
            else
            {
                temp = atoi(&command[1]);
                ...
                strcpy(command,g_history[(temp-1)%HISTORY_SIZE]);
            }
            current = strlen(command);
        }
        g_command_count++;
        strcpy(g_history[(g_command_count-1)%HISTORY_SIZE], command);
    }
}
...
int shell(int argc, char *argv[])
{
    ...
    while (42) {  //Program terminates when you enter the 'exit' command
        ...
        m_isValidCommand = fork();

        if (m_isValidCommand < 0) {

        //TODO: use perror() to indicate errors

            perror("errors");

        } else if (m_isValidCommand == 0) {
            ...
            m_isValidCommand = execvp(args[0], args);
